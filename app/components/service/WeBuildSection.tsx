...
const WeBuildSection = ({
  ...
  // Auto-scroll for IT Consulting variant
  const scrollContainerRef = useRef<HTMLDivElement>(null) // To access the scrollable div

  useEffect(() => {
    if (variant !== 'it-consulting') return;
    ...
    let currentIndex = 0;
    const scrollStep = 388 + 32; // Card width + gap

    const startAutoScroll = () => {
      intervalId = setInterval(() => {
        currentIndex++;
        // If we reach the end (or close to it), reset to 0
        // Determine max index based on items length
  ...
  const isITConsulting = variant === 'it-consulting';

  return (
    <div ref={weBuildSectionRef} className={`flex flex-col lg:flex-row px-[3%] py-8 lg:py-0 lg:justify-between lg:items-center w-full bg-custom-bg gap-8 lg:gap-0 ${isITConsulting ? 'min-h-[900px]' : 'min-h-[600px] sm:min-h-[700px] md:min-h-[800px] lg:h-[1000px]'}`}>
