...
const DualTextSection = ({
  ...
  return (
    <div ref={sectionRef} className="flex px-4 sm:px-6 md:px-8 flex-col lg:flex-row justify-between items-start gap-6 sm:gap-8 lg:gap-4 my-16 lg:my-24">
      <p className="dual-text-title text-3xl sm:text-4xl md:text-5xl lg:text-6xl">
        <span className="text-[#3B3B3D73]">{title}</span>
        <br className="lg:hidden" />
        <span className="text-[#000A1B]"> {titleHighlight}</span>
      </p>
      <p className="dual-text-description text-base sm:text-lg md:text-xl lg:text-2xl text-[#2D2C2C] leading-relaxed lg:max-w-[50%] pt-0 sm:pt-4 lg:pt-24">
        {description}
      </p>
    </div>
  )
